...
knitr::include_graphics("https://derekogle.com/NCGraphing/modules/zimgs/blackbear.jpg")
```
...
- Downloaded 'fwma-08-01-02_table s1.xls' Excel file from [Journal of Fish and Wildlife Management](https://www.fwspubs.org/doi/10.3996/012016-JFWM-003) and saved as [BlackBearsFL.csv]("https://raw.githubusercontent.com/droglenc/NCData/master/BlackBearsFL.csv").

--
- Loaded data into `flbb` data.frame.
...
- Constructed with `geom_histogram()`.
  ...
  - `breaks=`: Bin boundary values.
  - `boundary=`: Left-most boundary of first bin.
  - `center=`: Center of first bin.
  - `closed=`: Are bins closed on `"right"` (*default*) or `"left"`.
]
...
`r chunk_reveal("density2",break_type="non_seq",left_assign=leftAssign,split=splitPerc)`
...
p <- ggplot(data=flbb, mapping=aes(
  ...
  scale_y_continuous(name="Weight (kg)",
                     limits=c(0,250),breaks=seq(0,250,25),
                     expand=expansion(mult=c(0,0))) +
  scale_color_manual(values=c("red","blue")) + #BREAK3
  scale_fill_manual(values=c("red","blue")) + #BREAK4
  theme_bw() +
  theme(legend.position="none") #BREAK5
```

`r chunk_reveal("boxplot2",break_type="non_seq",left_assign=leftAssign,split=splitPerc)`
...
`r chunk_reveal("violin1",break_type="auto",left_assign=leftAssign,split=splitPerc)`
...
<font size="7">We will examine graphs for distributions of categorical data.</font>
