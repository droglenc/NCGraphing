...
A mathematics professor at Northland College has tracked the letter grade assigned to each student that completed his classes for the last five years. The data can be found in [MTHGrades.csv](https://raw.githubusercontent.com/droglenc/NCData/master/MTHGrades.csv). Their data are loaded below[^loaddata] and, for this exercise, "minus" and "plus" grades are converted to "whole"grades (use of `mapvalues()`). Letter grades and year were then converted to groupings (i.e., `factor()` within `mutate()`).[^copycode]
...
g2 <- ggplot(data=smry,mapping=aes(x=AY,y=perc,fill=Grade2)) +
  ...
  scale_y_continuous(name="Frequency of Students",expand=expansion(mult=c(0,0))) +
