...
[sports-reference.com](https://www.sports-reference.com/) has a wealth of information on major American sports. Statistics for Major League Baseball (MLB) players during the 2019 season, which are [in this file](https://derekogle.com/NCGraphing/modules/zdata/MLB19_Batting.csv), will be explored in this exercise.[^getdata] Make sure to load `tidyverse`.

```{r echo=TRUE, results="hide", message=FALSE, warning=FALSE}
library(tidyverse)
```

These data are loaded below and were restricted to only a few variables (i.e., `select()`), those players that had at least 200 at-bats in one league (i.e., `filter()`), and were in the top 100 with respect to batting "average" (i.e., `top_n()`). In addition, an age-group variable was created (i.e., `case_when()` within `mutate()`) and the `Lg` and `Age_grp` variables were converted to groupings (i.e., `factor()` within `mutate()`).
