...
ggplot(data=gli,mapping=aes(x=lake,y=max.cover)) +
  stat_summary(fun.data=mean_cl_normal,geom="pointrange",size=1,fatten=2) +
  scale_x_discrete(name="Great Lake") +
  scale_y_continuous(name="Mean Maximum Ice Coverage (%)") +
  theme_bw() +
  theme(panel.grid.major.x=element_blank())
...
ggplot(data=gli,mapping=aes(x=lake,y=max.cover,color=decade)) +
  stat_summary(fun.data=mean_cl_normal,geom="pointrange",size=0.5,fatten=1,
...
ggplot(data=sum2,mapping=aes(x=lake,y=mean)) +
  ...
  theme_bw() +
  theme(panel.grid.major.x=element_blank()) +
  geom_errorbar(aes(ymin=lsd,ymax=usd),width=0.15) +
