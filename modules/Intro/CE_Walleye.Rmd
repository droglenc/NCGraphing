...
The data are loaded below[^loaddata] and, for this exercise, reduced to only those fish captured in 2003 (i.e., `filter()`), the `loc`ation codes are changed from numbers to words (i.e., `mapvalues()` within `mutate()`), and `loc`ation is converted groupings (i.e., `factor()` within `mutate()`).
